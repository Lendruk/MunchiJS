import fs from "fs";
import { DynamicToken, ActionFunction, Action } from "./DynamicToken";
...
export default class TemplateEngine {
  ...
  constructor(viewDirectory: string) {
    ...
    this.tokens = [];
    const handlers: Map<string, Action> = new Map();
    handlers.set("FOR", {
      args: [],
      action: (options, variable: any, selector: "in" | "of", array: object[]) => {
        return options;
      },
    });
    // handlers.set("IF", { args: [], action: () => {} });
    this.defaultAction = new DynamicToken("{{", "}}", handlers);
